...
/*   Updated: 2026/05/22 12:59:41 by clwenhaj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int intersect_sphere(t_sphere *sphere, t_ray *ray, t_hit *hit)
{
    t_vec   oc;
    double  a;
    double  b;
    ...
    hit->t = root;
    hit->point = vec_add(ray->origin, vec_mult(ray->direction, root));
    hit->normal = vec_normalize(
				vec_sub(hit->point, sphere->center));
...
int intersect_plane(t_plane *plane, t_ray *ray, t_hit *hit)
{
    double  denominator;
    t_vec   diff;
    double  *t;
    ...
    *t = vec_dot(diff, plane->normal) / denominator;
    if (*t < EPSILON)
            return (0);
    hit->t = *t;
    hit->normal = vec_normalize(plane->normal);
...
int intersect_cone(t_cone *cone, t_ray *ray, t_hit *hit)
{
    t_vec   oc;
    double  a;
    double  b;
    ...
    *t = root;*/
    hit->t = root;
    hit->point = vec_add(ray->origin, vec_mult(ray->direction, root));
    t_vec  pc = vec_sub(hit->point, cone->apex);
    double m = vec_dot(pc, cone->axis);
    double  correction = 1.0 + (tan(cone->angle * PI / 180.0)
